...
pid_t *child_pids = NULL; // Global variable to hold child PIDs
...
void child_sigint_handler(int sig, siginfo_t *info, void *context) {
    sigint_received = 1;
}

void parent_sigint_handler(int sig, siginfo_t *info, void *context) {
    printf("Parent received SIGINT, forwarding to children\n");
...
void process_file(int n, char *filename) {
    ...
    child_pids = malloc(sizeof(pid_t) * n); // Allocate memory for child PIDs
    if (child_pids == NULL) ERR("malloc for children array");
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (pid == 0) {
            ...
            sigprocmask(SIG_BLOCK, &mask, &oldmask);  // Block SIGUSR1 and SIGINT
            ...
            for (int j = 0; j < end - start; j++) {
                if (sigint_received) {
                    printf("Child PID: %d received SIGINT, closing\n", getpid());
                    fclose(output_file);
                    free(child_data);
                    exit(EXIT_SUCCESS);
                }
